...
* OpenCore 0.7.6或更高版本的EFI文件已对Big Sur做了兼容性适配，理论上可以用在Big Sur及更早系统上，但未经本人测试。最后一个经过测试能够在Big Sur上正常使用的版本是0.7.4
...
* 麦克风及3.5mm音频输入接口无法工作，据说换成`VoodooHDA.kext`驱动就能正常使用麦克风和扬声器，但在Monterey系统下测试无效
* 长时间睡眠后唤醒异常（正在测试解决）
...
* 以非补丁的方式修复由于RTC错误而导致的启动自检失败，避免对其他系统造成潜在的影响，同时定制SSDT来尝试解决休眠问题（仍在测试，暂不放出测试EFI）
